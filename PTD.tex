...
Speculators can short the currency at a cost $t$, their payoff is described by $e* - f(\theta) - t$.
...
The introduction the risk for fundamentals: $ x \sim U[\theta - \epsilon, \theta + \epsilon] $.

\section{The Results}
\label{unc:results}
The main result from the paper is:
\begin{quotation}
	\noindent THEOREM 1: The is a unique $\theta *$ such that,
...
\begin{enumerate}
	...
	\item Authors claim that $u(x,\theta) < 0$ leads to $\pi (x) = 0$.
	\item This is assumption is false.
	\item Outside the certain risk area, the previously existing uncertainty remains.
	\item Multiple equilibria remain possible here.
\end{enumerate}

In the technical proof of the paper, we can pinpoint the non sequitur here:

\begin{quotation}
	\noindent For the next step,
	consider the strategy profile where every speculator attacks the currency if and only if
	the message $x$ is less than some fixed number $k$. \hspace{\stretch{1}}\parencite[p.~592]{morris1998unique}
\end{quotation}

The riskprofile of the speculator is being redefined here.
In the first iteration of the model,
speculators might decide the attack the currency without knowing what other investors would do.
In this second iteration,
speculators will only attack if they believe that there is a sufficient number of other attackers doing the same.
...
even in the absense of the quantifiable-risk induced certainty.
...
We can summarise the issues with the paper as follows:

\begin{enumerate}
	\item The introduction of risk adds information, rather than removing it.
	\item This does not in fact lead to a unique equilibrium.
	\item The assumption of a bounded risk interval is problematic.
\end{enumerate}
